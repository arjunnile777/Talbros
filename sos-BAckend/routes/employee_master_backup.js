          ...
          if (err) {
            ...
            db.query(
              `INSERT INTO employee_master (username, password, eid, role, name, status) VALUES (${db.escape(
                req.body.username
              )}, ${db.escape(hash)}, ${req.body.eid}, ${req.body.role}, ${db.escape(req.body.name)}, ${req.body.status})`,
              (err, result) => {
                if (err) {
                  throw err;
                  return res.status(201).send({
...
router.post('/getAll', customerMiddleware.create, (req, res, next) => {
    ...
    var query1 = 'select count(*) as TotalCount from employee_master where markDelete = 0';
    if(req.body.name) {
        query1 += ' AND name LIKE '+ db.escape('%'+req.body.name+'%');
    }
    if(req.body.role) {
        query1 += ' AND role LIKE '+ db.escape('%'+ parseInt(req.body.role) +'%')
    }
    if(req.body.eid) {
...
router.get('/getById/:id', customerMiddleware.create, (req, res, next) => {
    ...
    db.query('select * from employee_master where id=?',
    [req.params.id],
    function (error, results, fields) {
        if (error) throw error;
        console.log(results)
...
router.put('/update/:id', customerMiddleware.create, (req, res, next) => {
    ...
    db.query('UPDATE `employee_master` SET `name`=?,`role`=?, `eid`=?, `status`=?, `username`=?, `password`=? where `id`=?',
        [req.body.name, req.body.role, req.body.eid, req.body.status, req.body.username, req.body.password, req.params.id],
        function (error, results, fields) {
        if (error) throw error;
        if(results) {
            db.query(
...
router.put('/temp-delete/:id', customerMiddleware.create, (req, res, next) => {
    ...
    console.log(params);
    const current = Date.now();
    db.query('UPDATE `employee_master` SET `markDelete`=? where `id`=?',
        [1, req.params.id],
        function (error, results, fields) {
        if (error) throw error;
        return res.status(200).send({
...
router.delete('/delete', customerMiddleware.create, (req, res, next) => {
    ...
    console.log("======================");
        db.query('DELETE FROM `employee_master` WHERE `id`=?',
